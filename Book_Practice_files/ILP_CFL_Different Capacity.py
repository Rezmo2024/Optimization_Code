...
waste_vars = {}
for U in range(0,len(SW_LOCATIONS)):
        waste_vars[SW_LOCATIONS[U]] = pulp.LpVariable(f'W_{SW_LOCATIONS[U]}', 0, SW_CAPACITY[U])
...
for i in SW_LOCATIONS:
 prob += pulp.lpSum(assign_vars[(i, j)] * REQUIRE[j] for j in WorkStations) + waste_vars[i] == SW_CAPACITY[SW_LOCATIONS.index(i)] * use_vars[i]
prob.solve()
print(prob)
for i in SW_LOCATIONS:
