...
model +=sum(graph[node][neighbor] * variables[f'x_{node}_{neighbor}_{i}'] for node in graph for neighbor in graph[node] for i in Destination )
# Define Constraints
for i in Destination:
    ...
    for neighbor in graph[node]:
            ...
            for i in Destination:            
             if variables[f'x_{node}_{neighbor}_{i}'].varValue >0:
                ...
                if graph[node][neighbor] * variables[f'x_{node}_{neighbor}_{i}'].varValue > max:
                    max=graph[node][neighbor] * variables[f'x_{node}_{neighbor}_{i}'].varValue
            cost=cost+max
